...
import { generateFitnessRecipe } from '../services/geminiService';
...
export const RecipeDetailScreen = ({
    ...
    onUpgradeRecipe?: (recipe: Recipe) => void;
}) => {
    ...
    useEffect(() => {
        // If the recipe has no ingredients, it was generated by the PlanProposal schema
        if (recipe.ingredients && recipe.ingredients.length === 0 && userProfile && onUpgradeRecipe) {
            void upgradeRecipe();
        }
    }, [recipe.ingredients.length]);

    const upgradeRecipe = async () => {
        setIsUpgrading(true);
        setUpgradeStatus(language === 'en' ? 'Preparing the recipe details...' : 'Preparando os detalhes da receita...');
        
        try {
            const upgraded = await generateFitnessRecipe(
                recipe.name, 
                userProfile!.goal,
            ...
            if (upgraded) {
                // Keep the exact same ID, but override with full details
                onUpgradeRecipe?.({
                    ...recipe,
                    ...upgraded,
                    id: recipe.id, 
                    createdAt: recipe.createdAt,
                    imageUrl: recipe.imageUrl || upgraded.imageUrl
                });
            }
        } catch (e) {
            console.error("Failed to upgrade recipe", e);
        } finally {
    ...
    return (
        <View style={styles.container}>
            ...
            <LoadingOverlay visible={isUpgrading} message={upgradeStatus} />
